...
--
-- Host: localhost
-- Generation Time: Feb 19, 2026 at 12:34 PM
...
CREATE TABLE `parent_student_links` (
  ...
  `parent_user_id` int(11) NOT NULL COMMENT 'The parent/guardian user account',
  `student_id` int(11) NOT NULL COMMENT 'The linked student record',
  `relationship_name` varchar(50) DEFAULT NULL COMMENT 'e.g., Father, Mother, Guardian',
  `created_at` datetime DEFAULT current_timestamp(),
  `is_deleted` tinyint(1) DEFAULT 0,
  `deleted_at` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Links one parent/guardian account to multiple students';
...
ALTER TABLE `parent_student_links`
  ...
  ADD UNIQUE KEY `uq_parent_student` (`parent_user_id`,`student_id`),
  ADD KEY `idx_psl_parent` (`parent_user_id`),
...
--
ALTER TABLE `parent_student_links`
  ADD CONSTRAINT `fk_psl_parent` FOREIGN KEY (`parent_user_id`) REFERENCES `users` (`user_id`),
  ADD CONSTRAINT `fk_psl_student` FOREIGN KEY (`student_id`) REFERENCES `students` (`student_id`);
